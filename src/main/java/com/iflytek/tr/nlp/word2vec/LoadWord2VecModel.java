...
public class LoadWord2VecModel {
	private Logger logger = LoggerFactory.getLogger(LoadWord2VecModel.class);
	...
	public void loadGoogleModel(String path) throws IOException {
		logger.info("word2vec path = {}", path);
		File modelFile = new File(path);
		if (!modelFile.exists() ){
			logger.error("===============  word2vec模型不存在！"+path);
		}else{
			...
			try {
				...
				// WordSimilarity.class.getResourceAsStream("/" + path));
